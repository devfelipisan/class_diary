  ...
  if (data.option) {
    return (
      <Menu as="div" className="relative inline-block text-left">
        <div>
          <Menu.Button className="inline-flex w-full justify-center gap-x-1.5 rounded-md px-3 py-2 text-sm font-semibold text-white hover:bg-gray-700 hover:text-white">
            {data.name}
            <ChevronDownIcon
        ...
        >
          <Menu.Items className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
            <div className="py-1">
              {data?.option?.map((item: propsSubMenu) => (
                <Menu.Item key={item.subLabel}>
                  {({ active }) => (
                    <a
                      href={item.href}
